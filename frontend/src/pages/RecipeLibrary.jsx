...
export default function RecipeLibrary() {
  ...
  const isCustomRecipe = (recipe) => recipe?.source === 'user'
  ...
  useEffect(() => {
    ...
    async function loadRecipes() {
      ...
      try {
        ...
        if (!isMounted) return
        const adapted = (data.recipes || []).map((r) => {
          const ar = adaptRecipe(r)
          return { ...ar, source: r?.source ?? ar?.source }
        })
        setRecipes(adapted)
      } catch (err) {
        if (!isMounted) return
      ...
      <div className="page-content">
        ...
        {addOpen ? (
          ...
          >
            ...
            <div
              ref={addModalRef}
              className="relative z-10 flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-3xl bg-white shadow-soft"
            >
              <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-cream-200 bg-white p-6">
                <div>
              ...
              <form onSubmit={handleCreateRecipe} className="overflow-y-auto p-6">
                ...
                <div className="sticky bottom-0 mt-6 flex flex-col gap-3 border-t border-cream-200 bg-white pt-4 sm:flex-row sm:items-center sm:justify-between">
                  <button
                    type="button"
                    onClick={() => {
                      setAddOpen(false)
